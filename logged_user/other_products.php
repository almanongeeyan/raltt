<?php
// other_products.php
// Displays all product tiles that are NOT in the Geometric category (design_name != 'geometric')
...
if ($branch_id) {
	$stmt = $conn->prepare('
		SELECT p.product_id, p.product_name, p.product_price, p.product_description, p.product_image, p.product_type,
		       p.is_popular, p.is_best_seller
		FROM products p
		JOIN product_branches pb ON p.product_id = pb.product_id
		WHERE p.product_type = ? AND pb.branch_id = ? AND p.is_archived = 0
		ORDER BY p.product_name ASC
	');
	$stmt->execute(['other', $branch_id]);
...
<head>
	...
	<title>Other Products - RALTT</title>
...
<body>
	...
	<section class="min-h-screen py-0 px-0 bg-transparent">
		<div class="max-w-4xl mx-auto text-center pt-[50px] pb-6 px-4">
			<h1 class="text-4xl md:text-5xl font-black mb-3 tracking-tight text-[#7d310a]">Other Products</h1>
			<div class="text-base md:text-lg text-textlight mb-2 font-medium">Discover our unique tile designs outside the geometric collection.</div>
			<div class="text-sm text-textlight mb-6">All non-geometric tiles available at your branch.</div>
			<!-- Filter Buttons Only -->
		...
		<div class="max-w-7xl mx-auto mt-2 md:mt-6">
			<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
				<?php if (empty($filteredProducts)): ?>
					<div class="col-span-full text-center text-textlight py-8">No other products found for your branch.</div>
				<?php else: ?>
						...
						<div class="bg-white rounded-2xl overflow-hidden shadow-product transition-all duration-300 relative group border border-gray-100 hover:shadow-lg hover:-translate-y-2 hover:scale-105 flex flex-col p-0">
							<!-- Category/Badge -->
												...
												if (isset($prod['is_best_seller']) && $prod['is_best_seller'] == 1) {
													...
													$badgeLabel = isset($prod['design_name']) ? $prod['design_name'] : (isset($prod['product_type']) ? $prod['product_type'] : 'Other');
													$badge = '<span class="absolute top-4 left-4 bg-gradient-to-r from-[#7d310a] via-[#cf8756] to-[#e8a56a] text-white px-4 py-1.5 rounded-full text-sm font-black uppercase z-10 shadow-lg tracking-wider border-2 border-white" style="letter-spacing:0.08em;box-shadow:0 4px 18px #cf875655,0 2px 8px #e8a56a33;">' . htmlspecialchars($badgeLabel) . '</span>';
												}
												echo $badge;
												?>
