...
if ($branch_id) {
	...
	');
	$stmt->execute(['black and white', $branch_id]);
	...
	<section class="min-h-screen py-0 px-0 bg-transparent">
		<div class="max-w-4xl mx-auto text-center pt-[50px] pb-6 px-4">
			<h1 class="text-4xl md:text-5xl font-black mb-3 tracking-tight text-[#7d310a]">Black & White Tiles</h1>
			   <div class="text-base md:text-lg text-textlight mb-2 font-medium" style="color:#7d310a;">Discover our classic, elegant black & white tile designs for a timeless look.</div>
			   <div class="text-sm text-textlight mb-6" style="color:#7d310a;">All tiles in the Black & White design available at your branch.</div>
			<!-- Filter Buttons Only -->
		...
		<div class="max-w-7xl mx-auto mt-2 md:mt-6">
			<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
				<?php if (empty($filteredProducts)): ?>
					   <div class="col-span-full text-center text-textlight py-8" style="color:#7d310a;">No Black & White tiles found for your branch.</div>
				<?php else: ?>
					<?php foreach ($pagedProducts as $idx => $prod): ?>
						<div class="bg-white rounded-2xl overflow-hidden shadow-product transition-all duration-300 relative group border border-gray-100 hover:shadow-lg hover:-translate-y-2 hover:scale-105 flex flex-col p-0">
							...
							<div class="p-4 md:p-5 text-center flex flex-col items-center flex-1">
								   ...
								   <div class="text-xs md:text-sm font-medium text-textlight mb-2 line-clamp-3 min-h-[36px]" style="color:#7d310a;">
									   <?= htmlspecialchars($prod['product_description']) ?>
								   </div>
