...
if ($branch_id) {
    ...
    ');
    $stmt->execute(['minimalist', $branch_id]);
...
<head>
    ...
    <style>
        ...
        /* Override low-contrast text classes if any */
...
<body>
    ...
    <section class="min-h-screen py-0 px-0 bg-transparent">
        <div class="max-w-4xl mx-auto text-center pt-[50px] pb-6 px-4">
            ...
            <div class="text-base md:text-lg mb-2 font-medium" style="color:#7d310a;">Discover our sleek, simple, and modern tile designs for a clean look.</div>
            <div class="text-sm mb-6" style="color:#7d310a;">All tiles in the Minimalist design available at your branch.</div>
            
            <!-- Filter Buttons Only -->
            <div class="flex flex-wrap gap-2 justify-center mb-8 w-full max-w-2xl mx-auto">
                <a href="?" class="filter-btn px-6 py-3 rounded-full font-bold text-base <?= $filter=='' ? 'active' : 'bg-white' ?>" style="color:<?= $filter=='' ? '#fff' : '#7d310a' ?>;">All</a>
                <a href="?filter=popular" class="filter-btn px-6 py-3 rounded-full font-bold text-base <?= $filter=='popular' ? 'active' : 'bg-white' ?>" style="color:<?= $filter=='popular' ? '#fff' : '#7d310a' ?>;">Popular</a>
                <a href="?filter=bestseller" class="filter-btn px-6 py-3 rounded-full font-bold text-base <?= $filter=='bestseller' ? 'active' : 'bg-white' ?>" style="color:<?= $filter=='bestseller' ? '#fff' : '#7d310a' ?>;">Best Seller</a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto mt-2 md:mt-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                <?php if (empty($filteredProducts)): ?>
                    <div class="col-span-full text-center py-8" style="color:#7d310a;">No Minimalist tiles found for your branch.</div>
                <?php else: ?>
                    <?php foreach ($pagedProducts as $idx => $prod): ?>
                        <div class="bg-white rounded-2xl overflow-hidden shadow-product transition-all duration-300 relative group border border-gray-100 hover:shadow-lg hover:-translate-y-2 hover:scale-105 flex flex-col p-0">
                            ...
                            <div class="p-4 md:p-5 text-center flex flex-col items-center flex-1">
                                ...
                                <div class="text-xs md:text-sm font-medium mb-2 line-clamp-3 min-h-[36px]" style="color:#7d310a;opacity:0.95;"><?= htmlspecialchars($prod['product_description']) ?></div>
                                <div class="text-lg md:text-[1.25rem] font-extrabold mb-4" style="color:#7d310a;">₱<?= number_format($prod['product_price'], 2) ?></div>
            ...
            <div class="flex justify-center items-center gap-2 mt-10">
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <a href="?<?php if($filter)echo 'filter='.urlencode($filter).'&'; ?>page=<?= $i ?>" class="page-btn px-4 py-2 rounded-full font-bold text-base <?= $i == $page ? 'active' : 'bg-white' ?>" style="color:<?= $i == $page ? '#fff' : '#7d310a' ?>;border:1px solid #7d310a;">
                        <?= $i ?>
                    </a>
                <?php endfor; ?>
            </div>
