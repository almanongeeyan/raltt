...
<head>
    ...
    <title>Admin Branches Product Dashboard</title>
    ...
    <style>
        ...
        .product-count { background-color: #eff8ff; color: #1570ef; }
    </style>
</head>
            ...
            <div class="max-w-7xl mx-auto py-8 px-4">
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-800 flex items-center">
                        <i class="fas fa-boxes-stacked mr-3 text-blue-600"></i>Branches Product Dashboard
                    </h1>
                    <p class="text-gray-600 mt-2">View and manage products for each branch</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-8">
                    ...
                    <div class="branch-card bg-white rounded-2xl shadow-lg p-6 flex flex-col justify-between items-center border border-blue-100 relative mx-auto max-w-xs">
                        ...
                        <div class="text-sm text-gray-600 mb-4 flex-1 flex items-center justify-center product-count" id="branch-count-<?= $branch_id ?>">...</div>
                        <button class="view-products-btn <?= $isUserBranch ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-300 text-gray-400 cursor-not-allowed opacity-60' ?> font-medium px-5 py-2.5 rounded-lg shadow-sm transition flex items-center mt-2"
                ...
                <div id="productsModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-40 hidden">
                    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-8 relative">
                        <button id="closeProductsModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl focus:outline-none"><i class="fas fa-times"></i></button>
                        <h2 id="modalBranchTitle" class="text-2xl font-bold text-gray-800 mb-6 flex items-center"><i class="fas fa-store mr-2 text-blue-600"></i>Products for <span class="ml-2" id="branchName"></span></h2>
...
const viewBtns = document.querySelectorAll('.view-products-btn');
    ...
    btn.addEventListener('click', function(e) {
        ...
        if (branch !== userBranch) {
            Swal.fire({
                icon: 'error',
                title: 'Access Denied',
                text: 'You can only view products for your assigned branch (' + userBranch + ').',
                confirmButtonColor: '#8A421D',
            });
            e.preventDefault();
            return;
        }
        // Redirect to admin_addproduct.php with branch id
        window.location.href = 'admin_addproduct.php?branch_id=' + branchid;
    });
});
